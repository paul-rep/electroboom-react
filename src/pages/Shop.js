...
const Shop = () => {
  ...
  const [price, setPrice] = useState([50, 4500]);
  ...
  useEffect(() => {
    const delayed = setTimeout(() => {
      ...
      } else if (!text && Object.keys(filters).length > 0) {
        fetchProducts({ ...filters })
      } else {
  ...
  const handleCheck = (e) => {
    ...
    setFilters(prevFilters => ({...prevFilters, category: inTheState}));
    fetchProducts({...filters,category: inTheState});
  ...
  const handleBrand = (e) => {
    ...
    fetchProducts({...filters, brand: e.target.value});
  };
  ...
  const handleColor = (e) => {
    ...
    setFilters(prevFilters => ({...prevFilters, color: e.target.value}));
    
    fetchProducts({...filters, color: e.target.value});
  };
